...
export default function MemoryFeed({
  ...
  if (initialMemories.length === 0) {
    return (
      <>
        <div className="w-full px-5 sm:px-8 py-16" style={{ fontFamily: "'DM Sans', sans-serif" }}>
          <div className="mx-auto max-w-[420px] text-center">
            <div className="mx-auto mb-5 flex h-14 w-14 items-center justify-center bg-[#F5EDD8]">
              <BookOpen className="h-6 w-6 text-[#A9782F]" strokeWidth={1.5} />
            </div>
            <h3
            ...
            <div className="mt-6 flex flex-col sm:flex-row gap-3 justify-center">
              <button
                ...
                className="px-5 py-2.5 text-[13px] font-semibold text-white transition-all hover:shadow-lg active:scale-[0.98]"
        ...
        {showWriteModal && (
          <WriteMemoryModal familyId={familyId} onClose={() => setShowWriteModal(false)} onSuccess={() => { setShowWriteModal(false); router.refresh(); }} />
        )}
      </>
    );
  }
...
function WriteMemoryModal({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const { data: memory, error: memErr } = await supabase.from('family_memories').insert({ family_id: familyId, author_id: user.id, title: title.trim(), body: body.trim() }).select('id').single();
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center px-4">
      ...
      <div className="relative w-full max-w-[520px] max-h-[90vh] overflow-y-auto bg-white border border-[#EAD8B8] shadow-xl" style={{ fontFamily: "'DM Sans', sans-serif" }}>
        <div className="px-6 py-6 sm:px-8 sm:py-8 space-y-5">
          ...
          <div className="flex items-center gap-2 px-3 py-2 bg-[#F0EDE8] max-w-fit">
          ...
          <div>
            <label className="block text-[12px] font-medium text-[#4A4030] mb-1.5 tracking-[0.02em]">Title</label>
            <input type="text" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="e.g. Sunday mornings with Nan" maxLength={200} className="w-full border border-[#E0D6C8] px-3.5 py-2.5 text-[14px] text-[#181512] placeholder-[#C0B9AE] focus:outline-none focus:border-[#B8932A] focus:ring-1 focus:ring-[#B8932A]" style={{ background: '#FDFCFA' }} />
          </div>

          <div>
            <label className="block text-[12px] font-medium text-[#4A4030] mb-1.5 tracking-[0.02em]">Your memory</label>
            <textarea value={body} onChange={(e) => setBody(e.target.value)} placeholder="What happened? Who was there? How did it feel?" rows={5} className="w-full border border-[#E0D6C8] px-3.5 py-2.5 text-[14px] text-[#181512] placeholder-[#C0B9AE] focus:outline-none focus:border-[#B8932A] focus:ring-1 focus:ring-[#B8932A] resize-none leading-relaxed" style={{ background: '#FDFCFA' }} />
          </div>
          ...
          <button onClick={handleSubmit} disabled={!canSubmit || submitting} className="w-full py-3 text-[13px] font-semibold tracking-[0.04em] text-white transition-all hover:shadow-lg active:scale-[0.98] disabled:opacity-40 flex items-center justify-center gap-2" style={{ background: 'linear-gradient(135deg, #B8932A 0%, #C8A557 100%)' }}>
